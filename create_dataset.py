...
def read_parser():
    ...
    parser.add_argument(
        ...
        "--output",
        type=pathlib.Path,
        help="The directory/file path for storing the modified media in.",
        required=False,
    )
    ...
    parser.add_argument(
        ...
        "--image",
        type=pathlib.Path,
        help="The source image to apply a perturbation to.",
        required=False
    )
    ...
    parser.add_argument(
        ...
        "--video",
        type=pathlib.Path,
        help="The source video to apply a perturbation to.",
        required=False
    )
...
def media_files(
    ...
    def _is_media(x):
        is_video = mimetypes.guess_type(x)[0].startswith('video')
        is_image = mimetypes.guess_type(x)[0].startswith('image')
        return is_video or is_image

    yield from filter(_is_media, directory.rglob("*"))
...
def main():
    ...
    for file in media_files(source):
        print(file)
    # print(args)
