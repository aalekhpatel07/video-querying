...
def load_file(file_path: pathlib.Path) -> tp.Tuple[str, tp.Generator[dsc.CompactDescriptorVector, None, None]]:
...
class CompactDescriptorComparator:
    ...
    def match_descriptor(
        self,
        descriptor: dsc.CompactDescriptorVector,
        **kwargs
    ) -> tp.Generator[tp.Tuple[float, str], None, None]:
        """"""
    ...
    def match_video(
        ...
        **kwargs
    ) -> tp.Generator[tp.Tuple[str, float], None, None]:

        top = kwargs.get('top', 1)
        group_size = kwargs.get('group_size', 30)

        vectors = extractor.extract_from_video(
            fm.from_video_grouped(query, group_size),
            source_id=source_id
        )
    ...
    def match_file(
        ...
        **kwargs
        ...
        print(f"Matching {query.stem}{query.suffix} (stored as '{query_id}') against the database...")
